...
public final class InsertForm extends JFrame {
	...
	private final JButton setupExamBtn;
	...
	public InsertForm() {
		...
		mainLabel.setBounds(585, 5, 196, 29);
	...
	public boolean insertQuestion()
	{
	  ...
	  if(!onSubmitValiation())
	  {
		try
		{
			...
			cn=db.getConnection();
			st = cn.createStatement();
			rs = st.executeQuery("SELECT Category_id FROM  Category where Category_Name='"+((catCombo.getSelectedItem()).toString()).toUpperCase()+"'");
			if(rs.next())
				cId=rs.getLong(1);
			rs=st.executeQuery("SELECT eId FROM Exam_Questions order by eId DESC limit 1 ");
                        
                        System.out.println("i'm on line 495");
			if(rs.next())
				id=rs.getLong(1)+1;
			else
				id=1;
			...
			sql="INSERT INTO Exam_Questions (eId, Category_id, Question, Option1, Option2, Option3, Option4, Ans)values("+id+","+cId+",'"+QuestionText.getText().toUpperCase()+"','"+ans1Text.getText().toUpperCase()+"','"+ans2Text.getText().toUpperCase()+"','"+ans3Text.getText().toUpperCase()+"','"+ans4Text.getText().toUpperCase()+"',"+ans+")";
	...
	public void listLoad()
	{
		...
		try
		{
		    ...
		    if(rs.next())
		    	id=rs.getLong(1);
	        rs = st.executeQuery("SELECT Question, eId FROM  Exam_Questions where Category_id="+id+" order by eId");
	        list.removeAll();
	        int count=0;
	        while (rs.next())
	        {
	        ...
	        rs = st.executeQuery("SELECT COUNT(eId) FROM Exam_Questions");
	        String t="";
	        if(rs.next())
	        t=rs.getString(1);
	        totalLabel.setText("Total Question:"+t);
	...
	public void deleteListItem()
	{
	  ...
	  {
		int dialogButton = 0;
		int dialogResult = JOptionPane.showConfirmDialog (null, "Are you sure, You want to Delete","Delete",dialogButton);
		...
		System.out.println(lid+" eId "+t);
		try
		{
			st=cn.createStatement();
			System.out.println(st.executeUpdate("DELETE FROM Exam_Questions where eId ="+lid+""));
			list.removeAll();
			tmplist.removeAll();
			listLoad();
		}
		...
		if(!catCombo.getSelectedItem().equals("") && !list.isSelectionEmpty())
		  {
			...
			try
			{
				st=cn.createStatement();
				st=cn.createStatement();
				rs=st.executeQuery("SELECT * FROM Exam_Questions where eId ="+lid+"");
				if(rs.next())
				{
